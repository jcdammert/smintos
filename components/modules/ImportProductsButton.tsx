...
import { importGhlProductsAction } from "@/lib/actions";
...
export function ImportProductsButton() {
  ...
  function runImport() {
    setMsg(null);
    start(async () => {
      const res = await importGhlProductsAction();
      if (res.error) {
        setMsg(`Error: ${res.error}`);
      } else {
        setMsg(
          `Imported ${res.imported} product${res.imported === 1 ? "" : "s"} from GoHighLevel.`,
        );
        router.refresh();
      }
    });
  ...
  return (
    <div className="space-y-2">
      ...
      >
        {pending ? "Importing from GoHighLevel…" : "↓ Import products from GoHighLevel"}
      </button>
      {msg && (
        <p
